...
- [PHASE_STATUS.md](PHASE_STATUS.md) — **สถานะเฟส / ความคืบหน้างาน** (อัปเดตเป็นประจำ; snapshot ว่าอยู่เฟสไหน — ไม่แทนที่รายละเอียดแผนในไฟล์นี้)
...
- [SYSTEM_FLOWS.md](SYSTEM_FLOWS.md) — **flow ใช้งานแบบละเอียด** (บทบาท → login → import SAP → จบ flow + ระบบใหม่ใน repo)
- [SYSTEM_FLOWS_SIMPLE.md](SYSTEM_FLOWS_SIMPLE.md) — **flow แบบลูกค้าอ่านรู้เรื่อง** (เช่น ช่าง > login > แดชบอร์ด > … จนจบ)
...
- **ความคืบหน้าเฟส (อัปเดตบ่อย):** [`PHASE_STATUS.md`](PHASE_STATUS.md)
